...
<jstl:set var="owns"
          value="${logedActor.id == hike.route.user.userAccount.id}"/>

<jstl:set var="readonly"
          value="${(display || !owns || !edition) && hike.id != 0}"/>

<jstl:set value="route/list.do" var="backUrl"/>
<jstl:if test="${owns}">
    <jstl:set value="route/user/edit.do?routeId=${hike.route.id}" var="backUrl"/>
</jstl:if>
<div class="seccion w3-light-grey" <jstl:if test='${!readonly}'>id="dropbox" ondragover="return false"
    ...
    <form:form action="${requestUri}" modelAttribute="hike">
        ...
        <div class="row">
            <div class="col-100">
                <legend><spring:message code="label.route"/>: <jstl:out value="${hike.route.name}"/>
                </legend>
            </div>
        </div>
        <div class="row">
                        ...
                        <select id="difficulty" name="difficulty" class="w3-text-black"
                            ...
                            <jstl:forEach items="${difficultyLevels}" var="item">
                                <option value="${item}" id="${item}"
                                        <jstl:if test='${item==hike.difficulty}'>selected</jstl:if> >
